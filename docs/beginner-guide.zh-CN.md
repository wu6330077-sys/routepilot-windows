...
先确认两个端口是否显示“可以连接”。需要测速时点击“开始对比测速”；默认总测试流量约 20 MB。仪表盘只展示两条线路的数据和当前策略状态，不判断结果好坏。
...
回到线路测试仪表盘，点击“刷新启用状态”。状态区应显示 Edge 已经载入 RoutePilot 自动分流。是否再次测速由你决定；线路测速和策略是否启用是两个独立数据。
