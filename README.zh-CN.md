...
- Edge 是否正在使用 RoutePilot 自动分流；
- 主线路和高速线路的本地端口能否连接；
- 点击测速后，两条线路各自的单连接 Mbps；
- 高速线路速度除以主线路速度得到的比例。

仪表盘不会把结果标成“优化成功”或“优化失败”，由用户结合自己的线路、费用和用途判断。测速默认让每条线路下载两次 5 MB 测试文件，总测试流量约 20 MB；不点击测速时只检查本地状态，不产生测试下载。详细说明见 [线路测试仪表盘](docs/dashboard.zh-CN.md)。
...
安装前后都可以打开 `Open-Dashboard.cmd`。完成配置后点击“刷新启用状态”，即可看到当前 Edge 是否已经载入 RoutePilot 分流策略；是否重新测速由用户决定。
